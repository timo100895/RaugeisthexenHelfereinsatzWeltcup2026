...
export default function BulkMoveModal({ registrationIds, options, onClose, onDone }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    for (const id of registrationIds) {
      try {
        await moveRegistration(id, targetId);
      } catch (err) {
    ...
    if (failures.length > 0) {
      setError(
        `${registrationIds.length - failures.length} von ${registrationIds.length} erfolgreich verschoben. ` +
          `Fehler bei den restlichen: ${failures[0]}`
      );
      onDone();
    } else {
      onDone();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <form onSubmit={submit} className="w-full max-w-md rounded-2xl bg-white p-6">
        ...
        <p className="mb-4 text-sm text-gray-500">
          {registrationIds.length} {registrationIds.length === 1 ? 'Anmeldung' : 'Anmeldungen'} ausgewählt
        </p>

        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium">Neue Schicht für alle Ausgewählten</span>
          <select
            required
